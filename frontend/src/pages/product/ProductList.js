  ...
  export default function ProductList() {
    ...
    const handleCreateProduct = async () => {
      try {
        ...
        const productData = {
          ...newProduct,
          price: parseFloat(newProduct.price) * 100, // Đảm bảo giá được gửi đúng (theo đơn vị nhỏ hơn)
          sellerId, // Truyền sellerId từ localStorage
        };
        ...
        if (res.ok) {
          // Sau khi thêm, gọi lại API để lấy lại tất cả sản phẩm
          fetch(`http://localhost:5000/api/products?sellerId=${sellerId}`)
            .then(res => res.json())
            .then(data => setProducts(data))  // Cập nhật lại danh sách sản phẩm
            .catch(err => console.error(err));
    
          setShowModal(false);
          setNewProduct({ title: "", price: "", image: "", categoryId: "", description: "" });
        } else {
